...
const { sql: { sqlFind, sqlFindOrCreate, sqlCreate, sqlUpdate } } = require('sql-wizard')
...
const suggestUsername = async (pool, app, username, email) => {
  const slugBase = username || usernameFromEmail(email)

  // Find existing slugs
  const sqlString = `SELECT username FROM person_app WHERE app_id = ${app.id} AND username ILIKE '${slugBase}%';`
  ...
  await runDatabaseFunction(async (pool) => {
    await handleRestAction(async () => {
      if (method === 'OPTIONS') {
        ...
        // Person
        const { email: emailRaw, username: usernameRaw, firstName, lastName, country, feedback, ...metadata } = (typeof body === 'string') ? JSON.parse(body) : body
        ...
        if (requestType === 'login' || signupDirectUserWasFound) {
          ...
          const validUsername = await suggestUsername(pool, app, username, email)
          ...
          if (feedback !== undefined) {
            ...
            await sqlCreate(pool, 'feedback', feedbackData)
            const viewFeedbackUrl = `${process.env.APP_URL ?? ''}api/apps/${app.secret}/feedback`
            await postWebhook({
              text: `Feedback on ${app.name} from ${anonymizeEmail(person.email)} (#${person.id}):\n>_“${feedback}”_\n${viewFeedbackUrl}`
            })
          } else {
            // Webhook
            await postWebhook({
...
const getPersonInfo = async ({ pool, user_id }) => { // eslint-disable-line camelcase
  const sqlString = `SELECT
    ...
    metadata,
    purchase_session_id, subscription_session_id, credits FROM person_app
...
const updatePerson = async ({ pool, res, app, body, user_id }) => { // eslint-disable-line camelcase
  ...
  // Username unique
  if (username) {
    newPersonData.username = await suggestUsername(pool, app, username)
  }
  // Other fields
  ...
  await runDatabaseFunction(async (pool) => {
    await handleRestAction(async () => {
      ...
      if (method === 'GET') {
        ...
        await updatePerson({ pool, res, app, user_id })
      }
    }, res)
  })
}
