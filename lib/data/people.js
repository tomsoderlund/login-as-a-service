...
const suggestUsername = async (pool, username, email) => {
  const slugBase = username || email.match(/([a-z0-9]*)/gi)[0]
  ...
  await runDatabaseFunction(async (pool) => {
    await handleRestAction(async () => {
      if (method === 'OPTIONS') {
        ...
        let person, personApp, signupDirectUserWasFound

        if (requestType === 'signup-direct') {
          // Person
          const people = await sqlFind(pool, 'person', { email })
          if (people.length) signupDirectUserWasFound = true
        }
        ...
        if (requestType === 'login' || signupDirectUserWasFound) {
          ...
          if (requestType === 'signup-direct') results.token = generateToken(personApp, app)
...
const getPerson = async ({ pool, res, query, app, user_id }) => { // eslint-disable-line camelcase
  ...
  const user_id_numeric = person.person_id // eslint-disable-line camelcase
  delete person.person_id
  // Return results
  setAccessControlHeaders(res, 'GET')
  res.json({ ...person, user_id_numeric, ...metadata })
}
