  ...
  pool.connect(async (err, client, release) => {
    ...
    await handleRestAction(async () => {
      if (method === 'OPTIONS') {
        ...
        // Person
        const { email: rawEmail, username, firstName, lastName, country, ...metadata } = (typeof body === 'string') ? JSON.parse(body) : body
        if (!rawEmail) throw new Error('Person has no \'email\' property:400')
